...
export class Carousel extends React.Component<CarouselProps, CarouselState> {
    ...
    constructor(props: CarouselProps) {
        ...
        this.sortedBreakpoints = props.options?.breakpoints?.sort((a, b) => a.breakpoint - b.breakpoint) || [];
        this.activeBreakpoint = this.sortedBreakpoints.find(entry => entry.breakpoint >= document.body.clientWidth) || this.defaultBreakpoint;
    ...
    _setDimensions() {
        ...
        if (!visibleTrackContainer) {
            console.warn(`Ref not found for ${this.props.name} carousel container. Exiting sizing handler early.`);
            return;
        }
    ...
    handleControlClick(direction: keyof typeof Directions): void {
        if (!this.fullTrackRef.current) {
            console.warn('Ref not found for carousel tracks. Exiting control hadnler function early.')
            return;
        }

        const directionCoefficient = Directions[direction];
        const newTrackPosition = this.state.trackPosition + (this.state.cardWidth * directionCoefficient * this.activeBreakpoint.cardsToScroll);
        const maxTrackPosition = -(this.fullTrackRef.current!.clientWidth - (this.state.cardWidth * this.activeBreakpoint.cardsToShow));

        if (newTrackPosition < maxTrackPosition) {
            this.setState({
                ...this.state,
                trackPosition: FuzzyMaths.equals(this.state.trackPosition, maxTrackPosition, 1) ? 0 : maxTrackPosition,
            });
        } else if (newTrackPosition > 0) {
            this.setState({
                ...this.state,
                trackPosition: FuzzyMaths.equals(this.state.trackPosition, 0, 1) ? maxTrackPosition : 0,
            })
        } else {
            this.setState({
                ...this.state,
    ...
    render() {
        const cards = React.Children.toArray(this.props.children).map((child, index) => {
            return (
                <CarouselCard
